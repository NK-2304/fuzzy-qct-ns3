...
for algo in algos:
    for n in sources:
        ...
        t = re.search(r"Throughput\s*:\s*([\d\.]+)", res.stdout) # extracts the Mbps throughput.
        l = re.search(r"Loss Rate\s*:\s*([\d\.]+)", res.stdout) # extracts the loss rate percentage.
        d = re.search(r"Avg Delay\s*:\s*([\d\.]+)", res.stdout) # extracts the average delay in milliseconds.
